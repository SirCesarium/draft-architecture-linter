...
export async function activate(context: ExtensionContext) {
  ...
  // Match the naming convention for rc.7 binaries
  if (platform === 'linux') {
    binaryName = arch === 'arm64' ? 'sweet-lsp-linux-arm64' : 'sweet-lsp-linux';
  } else if (platform === 'win32') {
    binaryName = 'sweet-lsp-win.exe';
  } else if (platform === 'darwin') {
    binaryName = arch === 'arm64' ? 'sweet-lsp-macos-arm64' : 'sweet-lsp-macos';
  }

  // 1. Try local bin/ folder (production/installed)
  let serverModule = path.join(context.extensionPath, 'bin', binaryName);

  // 2. Fallback for development: check root target/release/
